...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('exportAlgoBtn')?.addEventListener('click', () => {
    ...
    const result = JSON.parse(localStorage.getItem('results'))?.greedy;   // đổi dp / branch
    if (!result) return alert('Chưa có kết quả.');
    exportSingleAlgoCSV(input, result, 'greedy');                          // đổi tên tương ứng
  });
  ...
  const { selectedItems, totalWeight, totalValue } = results.greedy;
